...
if ($num_phones_in != 2611) {
  die "Can only used 2611 -from (used $num_phones_in)."
}
...
while (<M>) {
  ...
  my $mapped_from = $1; #($num_phones_in == 80)? $1 : $2;
  my $mapped_to = ($num_phones_out == 64)? $2 : $1;
