...
if [ ! -f $lang/L_align.fst ]; then
  ...
  local/make_L_align.sh data/local/$tmp $lang $lang 2>&1 | tee $dir/log/L_align.log
fi
...
wav-to-duration scp:$data/wav.scp ark,t:- | awk '{print $1,$1,0,$2}' > segments
#local/align_text_to_segments.py -i $dir/align.txt > $data/segments
