...
mkdir -p data/local
local/timit_prep_flists.sh --corpus-dir=$CORPUS --dev-spk=$CONFDIR/dev_spk.list \
  --test-spk=$CONFDIR/test_spk.list --work-dir=data

# (3) Normalize the transcripts.
local/timit_norm_trans.pl -i data/local/train.trans -m $CONFDIR/phones.60-48-39.map \
  -to 48 > data/local/train.trans2;
for x in dev test; do
  local/timit_norm_trans.pl -i data/local/${x}.trans -m $CONFDIR/phones.60-48-39.map \
