...
help_message="$0: score the kwslist using the F4DE scorer from NIST
  Example:
    $0 [additional-parameters] <kws-data-dir> <kws-results-dir>
...
KWSEval -e $ecf -r $rttm -t $kwlist \
    -s $kwsoutputdir/kwslist.xml -p 0.001 -c -o -b -d -f ${kwsoutputdir}${f4de_prefix} || exit 1;

duration=`cat ${kwsoutputdir}${f4de_prefix}/sum.txt | grep TotDur | cut -f 3 -d '|' | sed "s/\s*//g"`

local/kws_oracle_threshold.pl --duration $duration --beta 99.9 ${kwsoutputdir}${f4de_prefix}/alignment.csv > ${kwsoutputdir}${f4de_prefix}/metrics.txt
